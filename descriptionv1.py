...
def get_hint(level):
    ...
    difficulty = "very easy" if level <= 3 else "moderate" if level <= 7 else "difficult"
        ...
        messages=[
            {"role": "system", "content": "You are a game master."},
            {
                ...
                "content": (
                    ...
                    "Be descriptive but not too revealing. Only use two sentences for the hint."
                ),
            },
        ],
    )
...
# Streamlit UI
st.title("Progressive Quiz Game")
